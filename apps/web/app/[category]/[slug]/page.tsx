...
import { getAllArticles, getFAQBySlug, getMDXDataBySlug } from '../../../lib/mdx';
...
export default async function ArticlePage({
  ...
  return (
    <>
      {/* Next.js App Router has no mechanism to inject <script> into <head> from a page component;
          body placement is the Next.js-recommended approach and is accepted by Google. */}
      <script
      ...
      <div className="layout-container py-12">
        <Breadcrumb category={category} slug={slug} title={article.frontmatter.title} />
            ...
            <header className="mb-8">
              ...
              <div className="flex flex-wrap items-center text-sm text-gray-400 gap-y-2 gap-x-4">
                ...
                <time dateTime={article.frontmatter.published_at}>
                  {new Date(article.frontmatter.published_at).toLocaleDateString('en-US', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric',
                  })}
                ...
                {article.frontmatter.last_reviewed && (
                  <>
                    <span className="hidden sm:inline">•</span>
                    <span>Last reviewed: {article.frontmatter.last_reviewed}</span>
                  </>
                )}
              </div>
            </header>
